...
import { Dimensions, LayoutAnimation, Platform, Animated } from "react-native";
...
export const AnimatedMap = Animated.createAnimatedComponent(MapView);
...
const ClusteredMapView = ({
    ...
    const [currentRegion, updateRegion] = useState(
        getSafe(() => restProps.region.__getValue(), restProps.region) ||
    ...
    return (
        <AnimatedMap
            {...restProps}
            ref={(map) => {
                if (map && map["_component"]) {
                    const reference = map._component;
                    restProps.mapRef(reference);
